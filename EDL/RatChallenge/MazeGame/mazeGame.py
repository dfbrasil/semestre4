...
# maze, h, w, posRatX, posRatY, posCheeseX, posCheeseY = load_maze_from_file('henrique.txt')
maze, h, w, posRatX, posRatY, posCheeseX, posCheeseY = load_maze_from_file('victor.txt')
# maze, h, w, posRatX, posRatY, posCheeseX, posCheeseY = load_maze_from_file('kadson.txt')
...
wallImage = pygame.image.load('/home/danieldfb/Repositório-NOVO/semestre4/EDL/RatChallenge/MazeGame/images/wall2.jpg')
...
def mark_visited(posRatX, posRatY): #valores em pixels
    ...
    visited_cells[posRatY // cell_size][posRatX // cell_size] = True #Índices
...
def clear_previous_movement(maze, rat_positions):
    ...
    if len(rat_positions) > 1:
        ...
        maze[prev_y // cell_size][prev_x // cell_size] = 0  # Índices - Marca a célula anterior como não visitada
...
pygame.mixer.music.load('/home/danieldfb/Repositório-NOVO/semestre4/EDL/RatChallenge/MazeGame/music.mp3')
...
while running:
    ...
    current_time = time.time() # intervalo entre os movimentos do rato
    
    # Realiza um movimento do rato a cada 0.05 segundos
    if current_time-last_move_time > 0.1 and not found_cheese:
    ...
    if path_display_time is not None:
        ...
        if current_ticks-path_display_time < 100000:  # Exibir as trilhas por 10 segundos
            for x, y in correct_path:
                pygame.draw.rect(screen, (255, 0, 0), pygame.Rect(x, y, cell_size, cell_size))
            for x, y, _ in rat_positions:
                pygame.draw.rect(screen, (0, 0, 255), pygame.Rect(x, y, cell_size, cell_size))
                
            file_path = '/home/danieldfb/Repositório-NOVO/semestre4/EDL/RatChallenge/MazeGame/hunter_way.txt'
            with open(file_path, 'w') as file:
                for x, y in correct_path:
                    file.write(f"{x},{y}\n")
            file_path = '/home/danieldfb/Repositório-NOVO/semestre4/EDL/RatChallenge/MazeGame/cheese_path.txt'
            with open(file_path, 'w') as file:
                for x, y, _ in rat_positions:
                    file.write(f"{x},{y}\n")
        else:
